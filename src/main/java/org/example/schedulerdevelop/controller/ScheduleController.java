...
public class ScheduleController {
    ...
    public ResponseEntity<ScheduleResponseDto> updateSchedule(@PathVariable Long scheduleId, @Valid @RequestBody ScheduleUpdateRequestDto updateDto) {
        return ResponseEntity.ok(scheduleService.updateSchedule(scheduleId, updateDto));
    }

    // 일정 삭제 API
    @DeleteMapping("/{scheduleId}")
    public ResponseEntity<String> deleteSchedule(@PathVariable Long scheduleId) {
        String title = scheduleService.deleteSchedule(scheduleId);
